function C=WideModelRO
%Function to simulate traffic on the wide bridge
%with the Turnig rule switched off
...
%---INTERACTION RULES AND POSITIONAL UPDATE-------------------------------
        ...
        for i=1:sP %For all ants currently on the bridge
           ...
           %Turn rule off
           if abs(ang)==A %If supposed to turn stop instead
               del=0;
           else
               del=1;
           end
            ...
            %Preliminary positions based on interactions
             ...
             Xprel=P(i,1)+del*P(i,3)*cos(P(i,4)+ang);
             Yprel=P(i,2)+del*P(i,3)*sin(P(i,4)+ang);
